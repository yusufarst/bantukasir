...
This document owns UI language, visual rules, prototype scope and review evidence. Refinement date: 20 September 2026. No prototype or visual design is approved. Candidate tokens must be tested with official assets. [01](01-PRD.md) owns dashboard information priorities, [14](14-BULK-IMPORT.md) imports, and [15](15-FINANCE-PROFITABILITY.md) financial meaning.

## Character and navigation

Professional, restrained, precise B2B/B2G operations. Quality comes from typography, spacing, alignment, hierarchy and interaction, not decorative cards/effects. Burgundy replaces the baseline's dark-blue accent assumption. Use it for identity, primary actions, focus and light selection; do not flood the canvas/sidebar. Semantic error red remains distinct through labels/icons. Default internal theme is neutral light; dark mode is not required. Default shadcn/Tailwind styling is not LATANSA's identity.
...
| Candidate neutrals | Canvas #F6F5F3, surface #FFFFFF, text #242326, secondary #615B60, divider #DED9DC |
| Candidate brand | Burgundy #74263D, hover #5E1E31, selected tint #F7EDF0; not asserted official logo colors |
| Candidate semantic colors | Success #246348, warning #8A5700, error #B42318; test contrast on actual surfaces |
...
Validate tokens, contrast and real-logo alignment at CP01. At most two density modes; do not carry compact desktop preference automatically to touch. Whitespace must separate meaningful groups.
...
| State | Behavior |
| --- | --- |
| Expanded desktop | Official wordmark plus always-visible collapse glyph inside one button; label **Ciutkan navigasi**, aria-expanded=true, aria-controls |
...
Assets are pending. Planned locations: assets/brand/source/ for owner-supplied colored/monochrome logos and optional banner; assets/brand/README.md for provenance, original name, variants, dimensions/checksum and usage permission; assets/brand/optimized/ for faithful optimization. Do not create replacement images or production asset directories now.

Never redesign, stretch, recolor or crop logo geometry. Use the appropriate official monochrome variant; if contrast fails, change the surface. Rail needs a legible official mark; if only a wordmark is supplied, obtain an authorized mark before CP01 approval. A wireframe may say **Logo resmi belum tersedia**, but is not a final asset.
...
CP01–CP04 establish Core before production. CP05 follows later. Advanced workflows revise their existing CP and pass Gate A when significant. Core approval is not blanket approval of future features. [11](11-BUILD-PLAN.md) alone owns task progress. No HTML is created in this documentation task.
