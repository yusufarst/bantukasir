...
| POS cash/transfer, own session open/close/history/report | Yes | No | Yes |
...
Owner checkout uses the owner's own active cash session, never impersonates another cashier. Cashier may open/close only an authorized available register as self and read only own sessions. SUPER_ADMIN manages logical registers, reads all reports and may perform an assisted close with physical count, recent authentication and mandatory reason while retaining the original responsible cashier. Operations has no checkout/session grant in this minimal baseline.
...
Audit important successful changes within the business transaction: Sale/payment/stock/product/price/config/user/correction/session open/session close. Dedicated owner Log/Riwayat filters date/user/type/product/session/reference. Operations sees safe stock history; cashier sees own Sale/session-report history only. Never log passwords, tokens, session cookies or private full payloads. Retain historical actor identities and original facts.
